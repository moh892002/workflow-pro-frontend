...
export const EmployeeManagement = () => {
  ...
  const handleDeleteClick = (emp: User) => {
      if (!user) return;

      if (isAdmin) {
          if (confirm("Are you sure you want to move this employee to the Recycle Bin?")) {
              DB.users.delete(emp.id, user);
              refresh();
          }
      } else if (isHR) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (editingId) {
        const existingUser = DB.users.getById(editingId);
        if (!existingUser) return;

        // Admin/HR cannot change others' profile pictures here
        // Preserve existing image
        const finalImage = existingUser.profileImage;
        ...
        DB.users.add({
            ...
            password: password, 
            profileImage: `https://ui-avatars.com/api/?name=${formData.fullName}&background=random`
        } as User);
    }
                ...
                <div className="flex flex-col md:flex-row gap-6">
                    <div className="flex flex-col items-center space-y-4">
                        <div className="relative group">
                            ...
                            {/* Admin Upload Logic Only for NEW users, editing existing users goes to Profile page */}
                            {!editingId && (
