...
export const Finance = () => {
  ...
  const refresh = () => {
      setRecords(DB.finance.getAll());
      setEmployees(DB.users.getAll());
  };

  const handleDelete = (id: string) => {
      if (!user) return;
      if (confirm("Move to Recycle Bin?")) {
          DB.finance.delete(id, user);
          refresh();
      }
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
      ...
      DB.logs.add({
          ...
          details: `Added ${newRecord.type} ($${newRecord.amount}) for ${newRecord.employeeName}`
      });
